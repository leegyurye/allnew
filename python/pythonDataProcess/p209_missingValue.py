import numpy as np
import pandas as pd
from pandas import Series, DataFrame

print('\n# 시리즈의 누락 데이터 처리')
print('# 원본 시리즈')
myseries = Series(['강감찬', '이순신', np.nan, '광해군'])
print(myseries)

print('\n# isnull() 함수 : NaN이면 True')
print(myseries.isnull())

print('\n# notnull() 함수 : NaN이면 False')
print(myseries.notnull())
print("-" * 40)

print('\n# notnull()을 이용하여 참인 항목만 출력')
print(myseries[myseries.notnull()])

print('\n# dropna()를 이용해 누락 데이터 처리')
print(myseries.dropna())

filename = 'excel02.csv'
myframe = pd.read_csv(filename, index_col='이름', encoding='utf-8')
print(myframe)

print('\n# dropna()를 이용해 누락 데이터 처리')
cleanded = myframe.dropna(axis=0)
print(cleanded)

print('\n# how="all"을 이용해 누락 데이터 처리')
cleanded = myframe.dropna(axis=0,how='all')
print(cleanded)

print('\n# how="any"을 이용해 누락 데이터 처리')
cleanded = myframe.dropna(axis=0,how='any')
print(cleanded)

print('\n# [영어] 칼럼의 NaN 제거 ')
print(myframe.dropna(subset=['영어']))

print('\n# 컬럼 기준, how="all"을 이용해 누락 데이터 처리')
cleanded = myframe.dropna(axis=1,how='all')
print(cleanded)

print('\n# 컬럼 기준, how="any"을 이용해 누락 데이터 처리')
cleanded = myframe.dropna(axis=1,how='any')
print(cleanded)

print('\n## before')
print(myframe)
myframe.loc[['강감찬', '홍길동'], ['국어']] = np.nan
print('\n## after')
print(myframe)

print(myframe.dropna(axis=1, how="all"))

print('\n ##thresh option')
print(myframe.dropna(axis=1, thresh=2))